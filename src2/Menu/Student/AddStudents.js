...
const AddStudent = (props) => {
    ...
    const handleSubmit = (values) => {
        try {
            axios.post('https://localhost:44323/api/Student', values).then(() => {
                setSave(true)
            })
            save ? AlertSuccess('Successfully added.') : AlertSuccess("Data not saved")
        }
        catch {
            AlertError("Invalid details")
        }
    }
    return <Fragment>  <div className="ml-2"><PageHeader header="Add Student" /></div><Card>
